...
//profile pic updated or not
    ...
    public function admin_birthplace(Request $request)
    {
        ...
        $data['user_birthplace_text'] = BirthplaceDetails::where('uid', $request->id)
            ->where('upload_type', 'text')->first();
    ...
    public function add_admin_birthplace_details(Request $request)
    {
        ...
        if ($request->upload_type == 'text') {
        ...
        } else if ($request->upload_type == 'update_text') {
            $birthplace = array(
                'title' => '',
                'file_url' => '',
                'uid' => $request->uid,
            ...
            //  dd($birthplace);
            DB::table('birthplace_details')->where('uid', $request->uid)
                ->where('upload_type', 'text')->update($birthplace);
            Session::flash('success', "Success!");
            return redirect('admin_birthplace')->with('status', "Update successfully");
        } else if ($request->hasFile('file_url')) {

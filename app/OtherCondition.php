...
class OtherCondition extends Model
{
  ...
  public static function get_top_qualifiers(){
    ...
    ->get();
    if($udata->count()>0){ 
      $pool_limit_prize=PoolOtherLimit::where('id',1)->first()->pool_prize;   
      $pool_collect_prize=PoolPrice::where('id',1)->first()->other; 
      ...
      foreach ($udata as  $value) {
        if(!empty($value->level))
        {
         ...
         // $active_users =$value->direct_members;
          ...
          $bonus= OtherCondition::where('level', $level)->first();
          if(!empty($bonus)){
  ...
  public static function distribute_bonus_for_other_jobs(){
     $success=false; 
     $pool_limit_prize=PoolOtherLimit::where('id',1)->first()->pool_prize;   
     $pool_collect_prize=PoolPrice::where('id',1)->first()->other; 
     $udata=OtherCondition::get_top_qualifiers();
     $total_qualifier=count($udata);
    // echo $pool_limit_prize;
    //print_r($udata);
   // die;
      ...
      foreach ($udata as  $user_id) {
              ...
              if(!empty($top3qualifier))
              {
                ...
                $share_amount= $pool_limit_prize/$qaulifier;
                 ...
                 if(!empty($top3qualifier[2])){
                  ...
                  $message="Other prize received as a top qualifier ";
                OtherCondition::send_bonus_prize_qualifier_email($user_id,$producer1,$producer2,$producer3,$affiliate_amount,$type,$term,$message); 
                 if(!empty($user_id))
                 {
                    if($user_id !=1){
                 ...
                 if($total_top_producer <3){
                   ...
                   $sponsor_name=User::get_user_info($user_id)->name;
                    $message1="Other prize received for top 3 qualified producers under affiliate <b>$sponsor_name </b>."; 
                   $uid=1;
                   $producer_amount1=$rem*$producer_amount;
                   
                  OtherCondition::send_bonus_prize_qualifier_email($uid,$producer1,$producer2,$producer3,$producer_amount1,$type,$term,$message1);
                   //$success=true;  
                 }
                foreach ($top3qualifier as $uid) {  
                   $sponsor_name=User::get_user_info($user_id)->name;
                    $message1="Other prize received for top 3 qualified producers under affiliate <b>$sponsor_name </b>."; 
                OtherCondition::send_bonus_prize_qualifier_email($uid,$producer1,$producer2,$producer3,$producer_amount,$type,$term,$message1);
       ...
       if($success==true){
          $remaining_balance=$pool_collect_prize-$pool_limit_prize;
            $update1=array('other'=>$remaining_balance);
...
// send_bonus_brize_qualifier_email
 ...
 public static function send_bonus_prize_qualifier_email($user_id,$producer1,$producer2,$producer3,$amount,$type,$term,$message){
     ...
     $q2= OtherReward::create($bonus_data);
      $table_name="other_rewards";

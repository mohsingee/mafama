                            ...
                            @if(!empty($user->code) || !empty($user->sponsor_id) )
								...
								@endif	
										<div class="col-md-12 text-center">
											<h4 style="font-size: 18px; font-weight: 600; color: black;">Profile Information</h4>
										</div>
								       <!--profile info-->
								   <div style="background-color: white; box-shadow: 0px 60px 100px -10px rgba(35, 47, 70, 0.06); width: auto; border-radius: 10px; padding: 20px; padding-top: 10px; padding-bottom: 10px; display: flex; flex-direction: column; align-items: center; justify-content: space-between;">
								<!--head-->
								<div style="flex; flex-direction: row; align-items: center; justify-content: center; margin-bottom: 30px;">
								    		<!--profile pic etc-->
										<div style="display:flex; flex-direction: column; align-items: center; justify-content: center; gap: 10px">
										    <div style="width: 80px; height: 80px; border-radius: 40px; background-color: black; display:flex; align-items: center; justify-content: center;">
										 <img src="{{ asset('public/images/affiliates/'.$user->image) }}" class="img img-responsive" style="max-width: 100%; max-height: 100%;">
										</div>
										<span style="display: flex; flex-direction: column; justify-content: center; align-items: center;">
										    <h4 style="text-transform: capitalize;" class="lbl_text">{{ isset($user->id)?$user->first_name:' '}} {{ isset($user->id)?$user->last_name:'' }}  </h4>
										    <p class="lbl_text" style="color: grey;">{{ isset($user->id)?$user->email:'' }}</p>
										</span>
								...
								<!--grid-->
								       <div class="row" style="padding-bottom:20px;" style="flex-wrap: wrap; align-items: center">
									    ...
									    @if(isset($user->id)?showcommuneName($user->birth_commune):'')
									    <div class="col-md-4">
											<div class="form-group">
												<label class="form-label" for="">Commune</label>
											<span class="lbl_text">{{ isset($user->id)?showcommuneName($user->birth_commune):'' }}  </span>
											</div>
										</div>
										@endif
										<div class="col-md-4">
										...
										<div class="col-md-4">
											<div class="form-group">
												<label class="form-label" for="">Commune</label>
											<span class="lbl_text">{{ isset($user->id)?showcommuneName($user->commune):'' }}  </span>
											</div>
										</div>
										<div class="col-md-4">
											<div class="form-group">
										...
										<div class="col-md-4">
											...
											<span class="lbl_text">{{ isset($user->id)?showarrondissementName($user->arrondissement):'' }}  </span>
											</div>
										</div>
										@endif
										...
										<div class="col-md-4">
											...
											<span class="lbl_text bfh-states" data-country="{{ isset($user->id)?$user->country:'' }}" data-state="{{ isset($user->id)?$user->state:'' }}"></span>
											</div>
										</div>
									...
									<!--</div>-->
									 ...
									 <div class="clearfix"></div>	
	 <div class="heading-title heading-dotted col-md-12 margin-bottom-20 margin-top-30 text-center">
        ...
        @endphp	
		 ...
		 <?php
		  ...
		  $icon='';
		   ...
		   ?>
            <div class="col-md-{{ $grid }} col-md-offset-{{ $offset }}" style="width: 35rem">
                ...
                <div class="pricingTable {{ $class }}">
                    ...
                    <div class="pricingTable-signup">
                    	<form></form>
        	<!--<form action="{{ url('business_redirect_paypal') }}" method="POST"  id="form-{{$plan->id}}" >-->
            ...
            <a href="javascript:void(0)" class="subscribe-plan" data-id="{{ $plan->id }}" data-order="{{ isset($user->id)?$user->id:'' }}" data-price="{{ $plan->monthly_fee }}">Continue</a>
             </form>
                    </div>
                </div>

            ...
            <div class="col-md-{{ $grid }} col-md-offset-{{ $offset }}">
                ...
                <div class="pricingTable {{ $class }}">
                    ...
                    <div class="pricingTable-signup">
                    	<form></form>
        	<form action="{{ url('update_current_plan') }}" method="POST"  id="form-{{$plan->id}}" >
        	 @csrf 	  
        	<input type="hidden" name="plan_id" value="{{ $plan->id }}">
        	<input type="hidden" name="fees" value="{{ $plan->monthly_fee }}">
        	<input type="hidden" name="user_id" value="{{ isset($user->id)?$user->id:'' }}">
            <a href="javascript:void(0)" class="subscribe-plan" data-id="{{ $plan->id }}" data-order="{{ isset($user->id)?$user->id:'' }}" data-price="{{ $plan->monthly_fee }}">Renew</a>
             </form>
                    </div>
                </div>
